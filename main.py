...
async def file_product():
    ...
    for row in range(3, max_row):
        ...
        print(val)
        cursor = connection_db.cursor()
        sql = "INSERT INTO products (no,brand,categories_en,categories_th,booktype_en,booktype_th,parts_no,model,serial_no,page_no,file_type,file_lang,price,sku,file_id,images) value (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%d,%s,%s,%s)"
        cursor.execute(sql,val)
        connection_db.commit()
